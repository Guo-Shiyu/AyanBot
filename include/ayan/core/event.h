...
namespace ayan
{
    using Event = std::variant<event::MetaEvent,
                               event::NoticeEvent,
                               event::MsgEvent,
                               event::RequestEvent>;

    using OptEvent = Optional<Event>;

    template <typename T>
    concept IsEvent = requires(T _)
    {
        {std::is_same_v<T, event::MetaEvent> ||
         std::is_same_v<T, event::NoticeEvent> ||
         std::is_same_v<T, event::RequestEvent> ||
         std::is_same_v<T, event::MsgEvent>};
    };
}
